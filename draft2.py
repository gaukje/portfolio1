...
def parse_num_bytes(num_str):
    """Convert the input string into the number of bytes."""
...
def format_summary_line(headers, data):
    """Format a summary line for the statistics table."""
...
def handle_client(connection, client_address):
    """Handle the client connection and measure the received data."""
    received_bytes = 0
...
def client_worker(server_ip, server_port, duration, interval, num_bytes=None):
    """The worker function for each parallel client connection."""
    try:
...
def print_interval(client_socket, start_time, sent_bytes, server_ip, server_port, interval, summary=False):
    """Print the interval statistics for the client."""
